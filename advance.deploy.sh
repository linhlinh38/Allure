...
if [ -d .git ]; then
  ...
  log "You may need to manually update your code before running this script."
fi

# Check if we need to rebuild the application image
if [ "$REBUILD_IMAGE" = "true" ]; then
  log "Rebuilding application image..."
  docker-compose -f docker-compose.app.yml build
  log "Application image rebuilt"
fi

# Run database migrations
log "Running database migrations..."
export MIGRATION_NAME=update_$(date +%Y%m%d%H%M%S)
GENERATE_MIGRATIONS=true MIGRATION_NAME=$MIGRATION_NAME docker-compose -f docker-compose.migrations.yml up --build --abort-on-container-exit
