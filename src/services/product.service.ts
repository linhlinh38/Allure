...
class ProductService extends BaseService<Product> {
  ...
  async getById(id: string) {
    const product =
      await this.queryGetProductAndRelationsAndGetCategoryDetails()
  ...
  async filteredProducts(
    ...
    } else if (filter.brandId && filter.brandId.length > 0) {
      ...
      for (const classification of body.productClassifications) {
        ...
        if (classification.title) {
          ...
          if (
            ...
            throw new BadRequestError("title of classification already exists");
          }
        }
      }
    }
